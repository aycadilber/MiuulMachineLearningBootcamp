...
# olmadıklarını tahmin edebilecek bir makine öğrenmesi modeli geliştirebilir misiniz?
...
#eşik değerleri hesaplama fonksiyonu
...
#eşik değerleri hesapladıktan sonra bir değişkende aykırı değer var mı sorusunu sormak için diğer fonksiyon
...
# hesaplanan eşik değerleri kullanarak bir değişkende eğer aykırı değer varsa
# bu aykırı değerleri hesaplanan eşik değeler ile değiştiren fonksiyon
...
100 * df["Outcome"].value_counts() / len(df)  # 1 ve 0 sınıfının oranı
...
# outcome değişkenini dışarıda bırakarak yapalım
...
# Standartlaştırma işlemlerinin 2 temel amacı :
# 1- modellerin değişkenlere eşit yaklaşmasını sağlamak
...
# robust scaler 'ın standart scaler'dan farkı aykırı değerlerden etkilenmiyor olmasıdır.
#  Her bir gözlem biriminden medyanı cıkarır. Medyan aykırı değerlere duyarsızdır.
...
# ROC AUC : farklı classification threshold degerlerine göre oluşacabilecek başarılarımıza yönelik genel bir metrik
...
# öyle bir işlem yapalım ki bu farklılıklar göz önünce bulundurulsun --> 10 katlı çapraz doğrulama
